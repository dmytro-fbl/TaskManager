...
export const EditProjectModal: React.FC<EditProjectModalProps> = ({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (budgetCap && budgetCap.trim() !== '') {
            const budgetValue = parseFloat(budgetCap);
            if (budgetValue < 0) {
                alert('Бюджет не може бути від`ємними');
                return;
            }
            if (budgetValue > 1000000) {
                alert('Бюджет занадто великий (максимум 1 000 000 годин)');
                return;
            }
        }
